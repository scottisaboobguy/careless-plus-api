...
async function fetchChannelVideos() {
  ...
  for (const line of lines) {
    ...
    const uploadTs = entry.timestamp ?? entry.release_timestamp ?? null;
    const isNew = uploadTs !== null && (Date.now() - uploadTs * 1000) < FOURTEEN_DAYS_MS;

    if (!rowMap.has(category)) rowMap.set(category, []);
    rowMap.get(category).push({ videoId, title, duration, thumbnailUrl: sd, thumbnailHd: hd, isNew });
  }
